...
class ArgumentParser(cfargparse.ArgumentParser):
    ...
    @classmethod
    def validate_custom_opts(cls, opt):
        if opt.num_emotion_classes == 0 and opt.emotion_emb_size != 0:
            raise ValueError('BOTH number of emotion classes and emotion embedding size need to be zero or positive.')
        if opt.num_emotion_classes != 0 and opt.emotion_emb_size == 0:
            raise ValueError('BOTH number of emotion classes and emotion embedding size need to be zero or positive.')
        if opt.num_emotion_classes != 0 and (opt.train_emotion_path == "" or opt.valid_emotion_path == ""):
            raise ValueError('A file path to input emotions must be provided for emotion embedding.')