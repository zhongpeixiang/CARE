...
from onmt.decoders.decoder import EDSDecoder
from onmt.translate.beam_search import BeamSearch
from onmt.translate.random_sampling import RandomSampling
from onmt.utils.misc import tile, set_random_seed
from onmt.modules.copy_generator import collapse_copy_scores
from onmt.utils.constants import id2emotion
...
class Translator(object):
    ...
    def from_opt(
        ...
        return cls(
            ...
            logger=logger,
            seed=opt.seed,
            emotion_topk_decoding_temp=opt.emotion_topk_decoding_temp)
    ...
    def rank_preds(self, preds, emotion_lexicon, message, stopwords):
        # preds (beam_size, #): a list of tokenized responses of varing lengths
        def select_emotional_responses(preds, emotion_lexicon):
            selected = []
            for pred in preds:
                for w in pred:
        ...
        def select_relevant_responses(preds, message, stopwords):
            selected = []
            for pred in preds:
                for w in pred:
                    if w in message and w not in stopwords:
                        selected.append(pred)
                        break
            if len(selected) == 0:
                return preds
        ...
        emotional_preds = select_emotional_responses(preds, emotion_lexicon)
        relevant_preds = select_relevant_responses(emotional_preds, message, stopwords)
    ...
    def translate(
            ...
            attn_debug=False,
            phrase_table="",
            tgt_emotion_shard=None,
            rerank=False,
            emotion_lexicon="",
            tgt_concept_embedding_shard=None,
            tgt_concept_words_shard=None):
        ...
        Args:
            ...
            tgt_emotion_shard: the shard of emotions, either a list of emotion labels or None
            tgt_concept_embedding_shard: the shard of concept embeddings, a list of list 
            tgt_concept_words_shard: the shard of concept words, a list of tuples (indices, scores, VAD_scores)
        ...
        for i in range(3):
            print(data.examples[i].src, data.examples[i].tgt)

        # add emotions into cur_dataset.examples
        if tgt_emotion_shard is not None:
            # print(len(data.examples), len(tgt_emotion_shard))
            assert len(data.examples) == len(tgt_emotion_shard)
            for ex, emotion in zip(data.examples, tgt_emotion_shard):
                ex.emotion = emotion
            emotion_field = Field(use_vocab=False, dtype=torch.long, sequential=False)
            data.fields["emotion"] = emotion_field
        
        if tgt_concept_embedding_shard is not None:
            assert len(data.examples) == len(tgt_concept_embedding_shard)
            for ex, tgt_concept_emb in zip(data.examples, tgt_concept_embedding_shard):
                ex.tgt_concept_emb = tgt_concept_emb
            tgt_concept_emb_field = Field(use_vocab=False, dtype=torch.float, sequential=False)
            data.fields["tgt_concept_emb"] = tgt_concept_emb_field

        if tgt_concept_words_shard is not None:
            # assert len(data.examples) == len(tgt_concept_words_shard)
            print(len(tgt_concept_words_shard[0]), len(data.examples))
            assert len(tgt_concept_words_shard[0]) == len(data.examples)
            assert len(tgt_concept_words_shard[1]) == len(data.examples)
            assert len(tgt_concept_words_shard[2]) == len(data.examples)
            # for ex, tgt_concept_words in zip(data.examples, tgt_concept_words_shard):
            ...
            for ex, tgt_concept_index, tgt_concept_score, tgt_concept_VAD_score in zip(data.examples, *tgt_concept_words_shard):
            ...
            data.fields["tgt_concept_index"] = tgt_concept_index_field
            data.fields["tgt_concept_score"] = tgt_concept_score_field
            data.fields["tgt_concept_VAD_score"] = tgt_concept_VAD_score_field


        if rerank:
            emotion_lexicon = load_pickle(emotion_lexicon)
            with open("./data/KB/Emotion-Lexicon/stopwords.pkl", "rb") as f:
        ...
        for i, batch in enumerate(data_iter):
            ...
            try:
                ...
                print(i)
                print(len(batch_data["predictions"]))
                print([len(pred) for pred in batch_data["predictions"]])
                print(len(batch_data["scores"]))
                print([len(score) for score in batch_data["scores"]])
                print(len(batch_data["attention"]))
                print([len(attn) for attn in batch_data["attention"]])
                print(len(batch_data["gold_score"]))
                print(len(batch_data["batch"].indices.data))
                continue
            ...
            for i, trans in enumerate(translations):
                ...
                if rerank:
                    emotion = id2emotion[batch.emotion[i].item()]
                    src_sent = [self.fields["src"].base_field.vocab.itos[w_id] for w_id in batch.src[0][:,i,0].tolist() if w_id != 1]
                    n_best_preds = self.rank_preds(trans.pred_sents[:self.n_best], emotion_lexicon[emotion], src_sent, stopwords)
    ...
    def get_emotion_mask(self, batch_emotion):
        ...
        emotion_mask = torch.zeros((batch_size, self.model.decoder.vocab_size)).to(batch_emotion.device)
        for i in range(batch_size):
            emotion_mask[i, self.model.decoder.emotion_vocab_indices[batch_emotion[i]]] = 1
        return emotion_mask
    ...
    def _translate_batch(
        ...
        if hasattr(batch, "emotion"):
            # expand emotion beam_size times
            batch_emotion = tile(batch.emotion, beam_size)
        
        batch_tgt_concept_emb = None
        if hasattr(batch, "tgt_concept_emb"):
            # expand emotion beam_size times
            batch_tgt_concept_emb = tile(batch.tgt_concept_emb, beam_size)
        
        batch_tgt_concept_words = None
        if hasattr(batch, "tgt_concept_index"):
            batch_tgt_concept_words = tile(batch.tgt_concept_index, beam_size), \
                tile(batch.tgt_concept_score, beam_size), tile(batch.tgt_concept_VAD_score, beam_size)
    ...
    def _score_target(self, batch, memory_bank, src_lengths,
        ...
        log_probs, attn = self._decode_and_generate(
            ...
            batch_emotion=batch_emotion, batch_tgt_concept_emb=batch_tgt_concept_emb,
            batch_tgt_concept_words=batch_tgt_concept_words)
