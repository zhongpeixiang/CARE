...
class TranslationBuilder(object):
    ...
    def from_batch(self, translation_batch):
        ...
        for b in range(batch_size):
            ...
            #     src_vocab, src_raw,
            #     preds[b][n] if len(preds[b])==self.n_best else [0 for i in range(self.n_best)], 
            #     attn[b][n] if len(attn[b])==self.n_best else [0 for i in range(self.n_best)])
            #     for n in range(self.n_best)]

            # clean translations
            
            if len(preds[b]) != self.n_best:
                # print(type(preds[b]), preds[b])
                # print(type(attn[b]), attn[b])
                # print(type(pred_score[b]), pred_score[b])
                
                while len(preds[b]) < self.n_best:
                    preds[b].append([0])
                    attn[b].append([])
                    pred_score[b].append(0)
                # attn[b] = [[] for i in range(self.n_best)]
                # pred_score[b] = [0 for i in range(self.n_best)]
