...
class OrderedIterator(torchtext.data.Iterator):
    ...
    def __iter__(self):
        ...
        """
        while True:
            self.init_epoch() # calls self.create_batches()
...
class DatasetLazyIter(object):
    ...
    def __init__(self, dataset_paths, fields, batch_size, batch_size_fn,
                 ...
                 tgt_emotions=None, src_concept_ids=None, tgt_concept_embedding=None,
                 tgt_concept_words=None):
        ...
        self.yield_raw_example = yield_raw_example
        self.pool_factor = pool_factor
        self.tgt_emotions = tgt_emotions
        self.src_concept_ids = src_concept_ids
        self.tgt_concept_embedding = tgt_concept_embedding
        self.tgt_concept_words = tgt_concept_words

    def _iter_dataset(self, path):
        logger.info('Loading dataset from %s' % path)
        cur_dataset = torch.load(path)
        logger.info('number of examples: %d' % len(cur_dataset))
        if self.tgt_emotions:
            assert len(self.tgt_emotions) == len(cur_dataset) # number of emotions equals to number of examples
        if self.src_concept_ids:
            assert len(self.src_concept_ids) == len(cur_dataset)
        if self.tgt_concept_embedding:
            assert len(self.tgt_concept_embedding) == len(cur_dataset)
        if self.tgt_concept_words:
            # assert len(self.tgt_concept_words) == len(cur_dataset)
            assert len(self.tgt_concept_words[0]) == len(cur_dataset)
            assert len(self.tgt_concept_words[1]) == len(cur_dataset)
            assert len(self.tgt_concept_words[2]) == len(cur_dataset)
        
        cur_dataset.fields = self.fields

        # add emotions into cur_dataset.examples
        if self.tgt_emotions is not None:
            for ex, emotion in zip(cur_dataset.examples, self.tgt_emotions):
                ex.emotion = emotion
            emotion_field = Field(use_vocab=False, dtype=torch.long, sequential=False)
            cur_dataset.fields["emotion"] = emotion_field

        # add src concept ids into cur_dataset.examples
        if self.src_concept_ids is not None:
            for ex, concept_ids in zip(cur_dataset.examples, self.src_concept_ids):
                ex.src_concept_id = concept_ids
            src_concept_id_field = Field(use_vocab=False, dtype=torch.long, sequential=False)
            cur_dataset.fields["src_concept_id"] = src_concept_id_field

        # add tgt concept embedding into cur_dataset.examples
        if self.tgt_concept_embedding is not None:
            for ex, tgt_concept_emb in zip(cur_dataset.examples, self.tgt_concept_embedding):
                ex.tgt_concept_emb = tgt_concept_emb
            tgt_concept_emb_field = Field(use_vocab=False, dtype=torch.float, sequential=False)
            cur_dataset.fields["tgt_concept_emb"] = tgt_concept_emb_field
        
        if self.tgt_concept_words is not None:
            # for ex, tgt_concept_words in zip(cur_dataset.examples, self.tgt_concept_words):
            #     ex.tgt_concept_index = tgt_concept_words[0]
            #     ex.tgt_concept_score = tgt_concept_words[1]
            #     ex.tgt_concept_VAD_score = tgt_concept_words[2]
            for ex, tgt_concept_index, tgt_concept_score, tgt_concept_VAD_score in zip(cur_dataset.examples, *self.tgt_concept_words):
                ex.tgt_concept_index = tgt_concept_index
                ex.tgt_concept_score = tgt_concept_score
                ex.tgt_concept_VAD_score = tgt_concept_VAD_score
            tgt_concept_index_field = Field(use_vocab=False, dtype=torch.long, sequential=False)
            tgt_concept_score_field = Field(use_vocab=False, dtype=torch.float, sequential=False)
            tgt_concept_VAD_score_field = Field(use_vocab=False, dtype=torch.float, sequential=False)
            cur_dataset.fields["tgt_concept_index"] = tgt_concept_index_field
            cur_dataset.fields["tgt_concept_score"] = tgt_concept_score_field
            cur_dataset.fields["tgt_concept_VAD_score"] = tgt_concept_VAD_score_field
            print("Finished building dataset iterators...")
        ...
        for batch in cur_iter:
            ...
            # print(batch)
            yield batch # this is the batch we want to modify
...
def build_dataset_iter(corpus_type, fields, opt, is_train=True, multi=False):
    ...
    # if input emotions are provided, load them
    tgt_emotions = None
    if opt.train_emotion_path:
        print("Loading tgt emotions from ", opt.train_emotion_path, opt.valid_emotion_path)
        if corpus_type=="train":
            tgt_emotions = read_emotion_file(opt.train_emotion_path)
        elif corpus_type=="valid":
            tgt_emotions = read_emotion_file(opt.valid_emotion_path)

    # if src concepts are provided, load them
    src_concept_ids = None
    if opt.train_src_concept_path:
        print("Loading src concept ids from", opt.train_src_concept_path, opt.valid_src_concept_path)
        if corpus_type=="train":
            src_concepts = read_src_concepts(opt.train_src_concept_path)
        elif corpus_type=="valid":
            src_concepts = read_src_concepts(opt.valid_src_concept_path)
        
        # convert concepts to ids
        src_concept_ids = []
        max_num_concepts = 10
        for concepts in src_concepts:
            src_concept_ids.append(convert_concepts_to_ids(concepts, opt.ent2id, max_num_concepts))

        # KG_emb_dim = opt.ent_emb.shape[1]
        # src_concept_embeddings = []
        # for concepts in src_concepts:
        #     if len(concepts) == 0:
        #         src_concept_embeddings.append(np.zeros(KG_emb_dim))
        #     else:
        #         src_concept_embeddings.append(np.stack([opt.ent_emb[opt.ent2id[c]] for c in concepts]))
    
    tgt_concept_embedding = None
    if opt.train_tgt_concept_embedding:
        print("Loading tgt concept embedding from ", opt.train_tgt_concept_embedding, opt.valid_tgt_concept_embedding)
        if corpus_type=="train":
            tgt_concept_embedding = load_pickle(opt.train_tgt_concept_embedding)
        elif corpus_type=="valid":
            tgt_concept_embedding = load_pickle(opt.valid_tgt_concept_embedding)
    
    tgt_concept_words = None
    if opt.train_tgt_concept_words:
        print("Loading tgt concept words from ", opt.train_tgt_concept_words, opt.valid_tgt_concept_words)
        if corpus_type=="train":
            tgt_concept_words = load_pickle(opt.train_tgt_concept_words)
        elif corpus_type=="valid":
            tgt_concept_words = load_pickle(opt.valid_tgt_concept_words)
        print("Finished loading tgt concept words")
    ...
    return DatasetLazyIter(
        ...
        src_concept_ids=src_concept_ids,
        tgt_concept_embedding=tgt_concept_embedding,
